...
from tensorflow.contrib.framework.python import ops
...
def leaky_relu(features, alpha=0.2, name=None):
  ...
  """
  with tf.name_scope(name, "LeakyRelu", [features, alpha]) as name:
    features = tf.convert_to_tensor(features, name="features")
    if features.dtype.is_integer:
      features = math_ops.to_float(features)
    alpha = tf.convert_to_tensor(alpha, dtype=features.dtype, name="alpha")
...
def bbox2mask(bbox, config, name='mask'):
    ...
    """
    #########################################################
    #For different mask shape insert here the masks images###
    #-> def mask_shape():				    #
    #        ?						    #
    #########################################################
...
def surface_attention(f, b, mask=None, ksize=3, stride=1, rate=1,
                         fuse_k=3, softmax_scale=10., training=True, fuse=True):
    """ Surface attention layer implementation.
